...
TRAIN_END:   str = "2023-12-31"   # reduced by 1 year → 8 years total
VAL_START:   str = "2024-01-01"   # extended to 2 years (2023–2024)
...
DROPOUT_RATE:  float = 0.334612739752567
RECURRENT_DROPOUT_RATE: float = 0
L2_FACTOR:     float = 4.89346566174196E-06
LEARNING_RATE:  float = 0.00709060148711602
...
MIN_COVERAGE: float = 0.5           # Minimum 25% dni w których model musi handlować
...
RUN_NAME:     str = "optuna_080 year 2025 test no rec drop"
BASE_OUT_DIR: str = os.path.join("fresh-walk-forward results", RUN_NAME)
...
def plot_feature_importance(
    ...
    label_map = {
        ...
        "btc_stoch_rsi_14": "BTC StochRSI",
        "btc_macd_hist":    "BTC MACD Hist.",
        "btc_atr_14_ratio": "BTC ATR Ratio",
    ...
    values = np.array([v for _, v, _ in importances], dtype=float)
    stds   = np.array([s for _, _, s in importances], dtype=float)
    colors = ["#3b7dbf" if v >= 0 else "#d9534f" for v in values]
 
    # ----- Compute data span so offsets scale with the data -----
    # Tips of error bars (where labels need to go BEYOND, not inside)
    right_tips = values + stds
    left_tips  = values - stds
    xmax_data  = max(0.0, float(right_tips.max()))
    xmin_data  = min(0.0, float(left_tips.min()))
    span       = max(xmax_data - xmin_data, 1e-6)
    gap        = span * 0.012   # space between error-bar cap and text label
    pad        = span * 0.18    # extra room at each side so text doesn't clip
 
    # ----- Plot -----
    fig, ax = plt.subplots(figsize=(12, max(5, len(labels) * 0.55)))
    y_pos = np.arange(len(labels))
    bars  = ax.barh(
        y_pos, values, xerr=stds, color=colors,
        edgecolor="white", height=0.72,
        error_kw=dict(ecolor="#333333", capsize=3, lw=1.0, alpha=0.85),
    )
    ax.set_yticks(y_pos)
    ax.set_yticklabels(labels, fontsize=10)
    ax.axvline(0, color="black", lw=1.0, ls="--", alpha=0.55, zorder=1)
    ax.set_xlim(xmin_data - pad, xmax_data + pad)
 
    # ----- Place value labels OUTSIDE the error bars -----
