...
LEARNING_RATE = 0.001
REG_STRENGTH  = 0.0001
THRESH_DIST   = 0.015    # Margines pewności (np. 0.02 oznacza progi 0.48 i 0.52)
...
def calculate_safe_metrics(X, y_true, returns, name):
    ...
    try:
        ...
        # --- POPRAWKA: DYNAMICZNE PROGI ---
        # Sprawdzamy, gdzie jest "środek" przekonań modelu
        model_median = np.median(preds_probs)
        
        # Jeśli model jest bardzo byczy (np. mediana 0.6), przesuwamy środek ciężkości
        # Ale żeby nie popsuć logiki, ustalmy 'center'
        center = model_median 
        
        upper = center + THRESH_DIST
        lower = center - THRESH_DIST
        
        print(f"     [DEBUG] {name} -> Mediana modelu: {model_median:.4f} | Próg Short: <{lower:.4f} | Próg Long: >{upper:.4f}")
        ...
        # 2. WIN RATE (Poprawiony)
        if n_trades == 0:
            win_rate = 0.0
        else:
            # Ważne: Porównujemy kierunek sygnału z faktycznym kierunkiem rynku
            # Sygnał 1 (Long) oczekuje y=1, Sygnał -1 (Short) oczekuje y=0
            
            # Konwertujemy sygnały na oczekiwaną klasę (1 dla Long, 0 dla Short)
            # Używamy tylko miejsc, gdzie signals != 0
            signals_active = signals[active_mask]
            expected_class = np.where(signals_active == 1, 1, 0)
            
            true_class = y_true[active_mask]
            
            # Teraz porównujemy: czy to co chcieliśmy (expected) zaszło (true)?
            win_rate = accuracy_score(true_class, expected_class) * 100
        ...
        import traceback
        traceback.print_exc() # Pokaże dokładniejszy błąd
