...
# --- 2. DANE ---
print(f"--- [PHOENIX V4: AGGRESSIVE SNIPER] Wczytywanie... ---")
...
# ZMIANA 1: Niższy próg ATR (Więcej okazji)
ATR_THRESHOLD = train_df['ATR_Pct'].median() * 0.40 # Tylko 40% średniej zmienności wymagane
print(f"Próg zmienności (ATR): {ATR_THRESHOLD:.4f}")

def get_position_v4(row):
    p = row['Prob']
    atr = row['ATR_Pct']
    
    # Filtr zmienności (Łagodniejszy)
    ...
    if p > 0.50:
        ...
        direction = 1
        certainty = (p - 0.5) * 2
        # ZMIANA 2: Agresywny mnożnik (x3)
        # Szybciej wchodzimy "All in"
        size = certainty * 3.0 
    else:
        # SHORT
        direction = -1
        certainty = (0.5 - p) * 2
        # Shorty też agresywniej (ale nadal z limitem 80% dla bezpieczeństwa)
        size = certainty * 2.0 * 0.8
        if certainty < 0.05: return 0.0 # Mniejszy próg odrzucenia

    return direction * min(size, 1.0) # Cap na 1.0 (bez lewarowania)

res['Position'] = res.apply(get_position_v4, axis=1)
...
res.to_csv('phoenix_v4_trades.csv')

plt.figure(figsize=(12, 6))
plt.plot(res.index, (1 + res['Actual_Ret']).cumprod(), label='Buy & Hold', color='grey', alpha=0.5)
plt.plot(res.index, (1 + res['Strat_Ret']).cumprod(), label=f'Phoenix V4 (WR {wr:.1f}%)', color='red')
plt.title(f'Phoenix V4 Performance (Aggressive Scaling)')
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('phoenix_v4_equity.png')