...
# --- 2. DANE ---
print(f"--- [PHOENIX V3: VOLATILITY FILTER] Wczytywanie... ---")
...
def prepare_dataset(df):
    ...
    # Zwracamy też ATR_Pct do filtrowania!
    return np.array(X), np.array(y), np.array(w), df.index[TIMESTEPS:], df['actual_return'].iloc[TIMESTEPS:].values, df['ATR_Pct'].iloc[TIMESTEPS:].values

X_train, y_train, w_train, _, _, _ = prepare_dataset(train_df)
X_val, y_val, w_val, _, _, _ = prepare_dataset(val_df)
X_test, y_test, w_test, idx_test, ret_test, atr_test = prepare_dataset(test_df)
...
# --- 6. STRATEGIA "SMART BIAS + VOLATILITY FILTER" ---
probs = model.predict(X_test).flatten()
res = pd.DataFrame(index=idx_test)
res['Actual_Ret'] = ret_test
res['Prob'] = probs
res['ATR_Pct'] = atr_test

# Obliczamy średnią zmienność z przeszłości (jako próg)
# Używamy średniej ATR z treningu jako benchmarku "normalnej zmienności"
ATR_THRESHOLD = train_df['ATR_Pct'].median() * 0.8 # Filtr: Musi być przynajmniej 80% typowej zmienności
print(f"Próg zmienności (ATR): {ATR_THRESHOLD:.4f}")

def get_position_v3(row):
    p = row['Prob']
    atr = row['ATR_Pct']
    
    # 1. FILTR ZMIENNOŚCI
    # Jeśli na rynku jest cisza (mała zmienność), nie wchodzimy.
    # Wtedy sygnały są często fałszywe.
    if atr < ATR_THRESHOLD:
        return 0.0

    # 2. LOGIKA KIERUNKU (z V1.5)
    if p > 0.50:
        # LONG
        direction = 1
        certainty = (p - 0.5) * 2
...
# Apply do całego wiersza (bo potrzebujemy i Prob i ATR)
res['Position'] = res.apply(get_position_v3, axis=1)
...
print(f"\n=== WYNIKI PHOENIX V3 (Vol Filter) ===")
print(f"Buy & Hold: {bh_tot:.2f}% | Sharpe: {bh_sh:.2f} | DD: {bh_dd:.2f}%")
print(f"Strategy:   {st_tot:.2f}%   | Sharpe: {st_sh:.2f} | DD: {st_dd:.2f}%")
print(f"Win Rate:   {wr:.2f}% (Trades: {len(active)} z {len(res)})")

res.to_csv('phoenix_v3_trades.csv')

plt.figure(figsize=(12, 6))
plt.plot(res.index, (1 + res['Actual_Ret']).cumprod(), label='Buy & Hold', color='grey', alpha=0.5)
plt.plot(res.index, (1 + res['Strat_Ret']).cumprod(), label=f'Phoenix V3 (WR {wr:.1f}%)', color='purple')
plt.title(f'Phoenix V3 Performance (ATR Filter > {ATR_THRESHOLD:.4f})')
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('phoenix_v3_equity.png')