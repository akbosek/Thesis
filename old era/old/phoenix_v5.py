...
print(f"--- [PHOENIX V5: SMA FILTER] Wczytywanie... ---")
...
# NOWOŚĆ: SMA 50 (Trend Filter)
data['SMA_50'] = data['Close'].rolling(window=50).mean()
# Feature: Odległość od średniej (żeby model też to widział)
data['Dist_SMA'] = (data['Close'] - data['SMA_50']) / data['SMA_50']

for i in range(1, 4):
    data[f'lag_{i}'] = data['log_return'].shift(i)

# Dodajemy Dist_SMA do features
features = ['log_return', 'RSI', 'ATR_Pct', 'Rel_Vol', 'MACD', 'Dist_SMA', 'lag_1', 'lag_2', 'lag_3']
...
# Zwracamy też Close i SMA_50 do strategii (jako surowe wartości)
def prepare_dataset(df):
    ...
    return (np.array(X), np.array(y), np.array(w), df.index[TIMESTEPS:], 
            df['actual_return'].iloc[TIMESTEPS:].values, 
            df['ATR_Pct'].iloc[TIMESTEPS:].values,
            df['Close'].iloc[TIMESTEPS:].values,   # Cena
            df['SMA_50'].iloc[TIMESTEPS:].values)  # Średnia
...
def get_position_v5(row):
    p = row['Prob']
    atr = row['ATR_Pct']
    price = row['Close']
    sma = row['SMA_50']
    
    # 1. Filtr Zmienności (V4)
    if atr < ATR_THRESHOLD: return 0.0

    # 2. NOWOŚĆ: Filtr Trendu (SMA 50)
    # Zabraniamy gry POD PRĄD głównego trendu
    trend_up = price > sma
    
    if p > 0.50: # Sygnał LONG
        if not trend_up: return 0.0 # Jeśli model chce Long, a cena pod średnią -> STOP
        ...
        if trend_up: return 0.0 # Jeśli model chce Short, a cena nad średnią -> STOP (w hossie shorty są groźne)
        
        certainty = (0.5 - p) * 2
        size = certainty * 2.0 * 0.8
        if certainty < 0.05: return 0.0
        return -1 * min(size, 1.0)

res['Position'] = res.apply(get_position_v5, axis=1)
...
res.to_csv('phoenix_v5_trades.csv')

plt.figure(figsize=(12, 6))
plt.plot(res.index, (1 + res['Actual_Ret']).cumprod(), label='Buy & Hold', color='grey', alpha=0.5)
plt.plot(res.index, (1 + res['Strat_Ret']).cumprod(), label=f'Phoenix V5 (WR {wr:.1f}%)', color='gold')
plt.title(f'Phoenix V5 Performance (Trend Filter)')
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('phoenix_v5_equity.png')