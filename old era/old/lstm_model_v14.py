...
# --- 2. WCZYTANIE ---
print(f"--- [V14: STACKED LSTM & 30D] Wczytywanie: {INPUT_FILE} ---")
...
# --- 3. FEATURES (Rozbudowane dla 30 dni) ---
data['log_return'] = np.log(data['Close'] / data['Close'].shift(1))

# Dłuższe średnie dla szerszego kontekstu
...
# --- POPRAWKA BOLLINGER BANDS (używamy iloc zamiast nazw) ---
bb = ta.bbands(data['Close'], length=20, std=2)
# pandas_ta zwraca: [Lower, Mid, Upper, Bandwidth, Percent]
# Bierzemy ostatnią (Percent) i przedostatnią (Width)
data['BB_Pct'] = bb.iloc[:, 4]   # Percent B
data['BB_Width'] = bb.iloc[:, 3] # Bandwidth
# ------------------------------------------------------------

# Lags (Pamięć krótkoterminowa)
lags = 5
...
# --- 5. SKALOWANIE ---
scaler_std = StandardScaler().fit(train_df[features]) # Drzewa
scaler_mm = MinMaxScaler((-1, 1)).fit(train_df[features]) # LSTM
...
# --- 6. MODELOWANIE (Architecture V14: Stacked LSTM) ---

print("\n--- Training LSTM V14 (Deep Stack) ---")
lstm_model = Sequential([
    Input(shape=(X_train_lstm.shape[1], X_train_lstm.shape[2])),
    
    # WARSTWA 1: Bidirectional, szeroka (64), przekazuje sekwencje dalej
    Bidirectional(LSTM(64, return_sequences=True, kernel_regularizer=l2(0.001))),
    Dropout(0.3),
    
    # WARSTWA 2: Zwykła, węższa (32), kondensuje wiedzę
...
def analyze_model(name, probs, returns, t_short, t_long):
    # DLA LSTM: Agresywny (bez strefy neutralnej)
    if name == 'LSTM':
        pos = np.where(probs > 0.50, 1, -1)
        size = np.ones_like(pos)
    else:
        # Drzewa: ze strefą neutralną
    ...
    # Win Rate (tylko aktywne transakcje)
    ...
    return {'Model': name, 'Win Rate': win_rate, 'Return %': total_ret, 'Sharpe': sharpe, 'Max DD': max_dd, 'Equity': cum_ret}

res_bh = {'Model': 'Buy & Hold', 'Return %': ((1+ret_test).cumprod()[-1]-1)*100, 'Sharpe':0, 'Max DD':0, 'Win Rate':0, 'Equity': (1+ret_test).cumprod()}
...
results_table = pd.DataFrame([
    {k:v for k,v in res_bh.items() if k != 'Equity'},
    {k:v for k,v in res_lstm.items() if k != 'Equity'},
    {k:v for k,v in res_rf.items() if k != 'Equity'},
    {k:v for k,v in res_xgb.items() if k != 'Equity'}
])

print(results_table.round(2).to_string(index=False))

# Export transakcji LSTM dla Ciebie
df_details['LSTM_Prob'] = probs_lstm
df_details['LSTM_Pos'] = np.where(probs_lstm > 0.5, 1, -1)
df_details['LSTM_Win'] = (df_details['LSTM_Pos'] == np.where(ret_test > 0, 1, -1)).astype(int) # Czy zgadł kierunek?
df_details['LSTM_PnL'] = df_details['LSTM_Pos'] * ret_test
df_details.to_csv('v14_detailed_trades.csv')
print("\n>>> Zapisano szczegóły transakcji LSTM do: v14_detailed_trades.csv")
...
plt.title(f'V14: Stacked LSTM (30 Days Lookback)')
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('v14_results.png')
print("Wykres zapisano: v14_results.png")