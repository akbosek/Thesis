...
# Imports
from sklearn.preprocessing import StandardScaler # <-- ZMIANA: StandardScaler zamiast MinMaxScaler
...
# --- Step 1: Parameters ---
TICKER = 'BTC-USD'
TIMESTEPS = 15 # Zmniejszamy okno czasowe (mniej szumu z dalekiej przeszłości)
PROB_THRESHOLD = 0.50 # <-- ZMIANA: Zmuszamy model do decyzji (0.5)
...
# Zamiast zmienności, dodajmy SMA (odległość od średniej)
data['SMA_20'] = data['Close'] / data['Close'].rolling(window=20).mean() - 1
macd = ta.macd(data['Close'])
data['MACD'] = macd.iloc[:, 0]

# Lags
lags = 3 # Zmniejszamy liczbę lagów (mniej szumu)
for lag in range(1, lags + 1):
    data[f'log_return_lag_{lag}'] = data['log_return'].shift(lag)

features = ['log_return', 'RSI', 'SMA_20', 'MACD'] + [f'log_return_lag_{i}' for i in range(1, lags + 1)]
...
# --- Step 5: Scaling (STANDARD SCALER) ---
# StandardScaler jest lepszy dla sieci neuronowych w finansach (radzi sobie z outlierami)
...
# --- Step 7: Modeling V5 (Simplified) ---

# == LSTM: Mniejszy i prostszy ==
print("\n--- Training LSTM V5 (Simplified) ---")
lstm_model = Sequential()
lstm_model.add(Input(shape=(X_train.shape[1], X_train.shape[2])))
# Zmniejszamy do 32 neuronów. Mniej znaczy lepiej przy szumie.
lstm_model.add(LSTM(units=32, return_sequences=False)) 
lstm_model.add(Dropout(0.4))
lstm_model.add(Dense(1, activation='sigmoid'))

optimizer = Adam(learning_rate=0.001) # Standardowy LR
...
# == RF & XGB (Standard) ==
print("\n--- Training RF & XGB ---")
# Używamy sprawdzonych parametrów, nie tracimy czasu na szukanie w tym kroku
rf_model = RandomForestClassifier(n_estimators=200, max_depth=5, min_samples_split=10, class_weight='balanced', random_state=42)
rf_model.fit(X_train_flat, y_train)

xgb_model = XGBClassifier(n_estimators=200, max_depth=3, learning_rate=0.05, scale_pos_weight=1.5, random_state=42)
...
plt.title('LSTM V5 Learning Curve')
plt.legend()
plt.savefig('v5_lstm_learning.png')
plt.close()

# --- Backtest (No Threshold) ---
...
metrics = [
    ...
    get_metrics(results_df['lstm_equity'], "LSTM V5"),
    get_metrics(results_df['rf_equity'], "Random Forest"),
    get_metrics(results_df['xgb_equity'], "XGBoost")
]
...
plt.plot(results_df['lstm_equity'], label='LSTM V5', linewidth=2)
plt.plot(results_df['rf_equity'], label='Random Forest', linewidth=2)
plt.plot(results_df['xgb_equity'], label='XGBoost', linewidth=2)
plt.title(f'Backtest V5 (StandardScaler & Forced Trade)')
plt.ylabel('Equity')
plt.legend()
plt.grid(True)
plt.savefig('v5_backtest_results.png')
plt.close()

print("Done. Check v5_backtest_results.png")