...
OUT_ROOT = Path("May the God save my ass results") / "walk_forward"
...
def _rolling_quantile(p: np.ndarray, q: float, window: int,
    ...
    """Rolling quantile, optionally seeded with a history array. The seed
    is prepended for the rolling computation, then trimmed off — so day 1
    of the test period uses the last `window` days of `seed` as context."""
    if seed is not None and len(seed) > 0:
        full = np.concatenate([seed, p])
        roll = pd.Series(full).rolling(window=window, min_periods=1).quantile(q).values
        return roll[len(seed):]
    return pd.Series(p).rolling(window=window, min_periods=1).quantile(q).values
