...
# ==============================================================================
# 7. RAPORTOWANIE (WSZYSTKIE WYKRESY + SHORT)
# ==============================================================================
def generate_report(X, y_true, prices, name, threshold):
    print(f"\n>>> RAPORT: {name} <<<")
    probs = model.predict(X, verbose=0).flatten()
    
    # KROK 1: Decyzja surowa (0/1)
    raw_preds = (probs > threshold).astype(int)
    
    # KROK 2: Konwersja na Pozycję (-1/1)
    ...
    plt.title(f'{name} - Krzywa Kapitału')
    plt.legend(); plt.grid(True, alpha=0.3)
    plt.savefig(f"{OUTPUT_DIR}/{name}_1_Equity.png")
    plt.close()
    
    # --- WYKRES 2: Sygnały na Cenie (PRZYWRÓCONY) ---
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Reds', xticklabels=['SHORT', 'LONG'], yticklabels=['SPADEK', 'WZROST'])
    plt.title(f'{name} Macierz Decyzji')
    plt.ylabel('Prawda'); plt.xlabel('Model')
    plt.savefig(f"{OUTPUT_DIR}/{name}_3_Matrix.png")
    plt.close()

    # --- WYKRES 4: Lorenz (PRZYWRÓCONY) ---
    ...
    # --- WYKRES 5: Histogram Pewności (PRZYWRÓCONY) ---
