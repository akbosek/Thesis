...
# Importujemy wszystkie silniki (Optymalizatory)
from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import LSTM, Dense, Dropout, Input, BatchNormalization
from tensorflow.keras.callbacks import EarlyStopping, ReduceLROnPlateau, ModelCheckpoint
from tensorflow.keras.regularizers import l2
from tensorflow.keras.optimizers import Adam, Adamax, Nadam, RMSprop, SGD, Adagrad, Adadelta, Ftrl
...
print(f" > Optymalizator: {HP_OPTIMIZER.get_config()['name']}")
...
# ==============================================================================
# 7. RAPORTOWANIE (POPRAWIONE ETYKIETY SHORT/LONG)
# ==============================================================================
def generate_report(X, y_true, prices, name, threshold):
    ...
    longs = np.sum(preds == 1)
    shorts = np.sum(preds == 0)
    
    equity = [100.0]
    market = [100.0]
    real_returns = prices.pct_change().shift(-1).dropna()
    min_len = min(len(preds), len(real_returns))
    
    active_preds = preds[:min_len]
    ...
    for i in range(min_len):
        ...
        # --- LOGIKA LONG / SHORT ---
        if active_preds[i] == 1: 
            # LONG: Zarabiasz na wzrostach
            equity.append(equity[-1] * (1 + ret)) 
        else: 
            # SHORT: Zarabiasz na spadkach (odwrócony zwrot)
            equity.append(equity[-1] * (1 - ret)) 
    ...
    # --- WYKRESY (Z POPRAWIONYM NAZEWNICTWEM) ---
    ...
    plt.title(f'{name} - Krzywa Kapitału (Agresywna)')
    plt.legend(); plt.grid(True, alpha=0.3)
    plt.savefig(f"{OUTPUT_DIR}/{name}_1_Equity.png")
    plt.close()
    
    # 2. Matrix (SHORT / LONG - Naprawione etykiety)
    plt.figure(figsize=(6, 5))
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
                xticklabels=['SHORT', 'LONG'], 
                yticklabels=['SHORT', 'LONG'])
    plt.title(f'{name} Confusion Matrix')
    plt.xlabel('Twoja Pozycja (0=Short, 1=Long)')
    plt.ylabel('Prawda (0=Spadek, 1=Wzrost)')
    plt.savefig(f"{OUTPUT_DIR}/{name}_3_Matrix.png")
    plt.close()
    
    # 3. Sygnały (SHORT / LONG)
    plt.figure(figsize=(14, 7))
    plt.plot(dates, active_prices, label='Cena', color='black', alpha=0.5)
    b_idx = [i for i,x in enumerate(active_preds) if x==1]
    s_idx = [i for i,x in enumerate(active_preds) if x==0]
    
    if len(b_idx)>0: plt.scatter(dates[b_idx], active_prices[b_idx], marker='^', color='green', s=20, label='LONG')
    # Czerwony trójkąt = SHORT
    if len(s_idx)>0: plt.scatter(dates[s_idx], active_prices[s_idx], marker='v', color='red', s=20, label='SHORT')
    ...
    plt.axvline(threshold, color='red', linestyle='--', label='Próg Decyzji')
    plt.title(f'{name} - Pewność Modelu')
    plt.legend()
