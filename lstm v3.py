...
# --- Step 9: Backtest with THRESHOLD (KROK 4) ---
...
# Funkcja strategii z progiem
def strategy_with_threshold(probs, returns, threshold=0.55):
    # Long (1) jeśli pewność > threshold
    # Short/Cash (0) jeśli pewność < (1 - threshold) -> tutaj upraszczamy:
    # Jeśli model jest pewien wzrostu (>0.55) -> KUPUJ
    # W przeciwnym razie -> GOTÓWKA (0)
    
    positions = np.where(probs > threshold, 1, 0)
    return positions * returns

results_df['lstm_equity'] = strategy_with_threshold(y_pred_prob_lstm, results_df['actual_return'], PROB_THRESHOLD).cumsum().apply(np.exp)
results_df['rf_equity'] = strategy_with_threshold(y_pred_prob_rf, results_df['actual_return'], PROB_THRESHOLD).cumsum().apply(np.exp)
results_df['xgb_equity'] = strategy_with_threshold(y_pred_prob_xgb, results_df['actual_return'], PROB_THRESHOLD).cumsum().apply(np.exp)
...
plt.plot(results_df['lstm_equity'], label=f'LSTM (Thresh {PROB_THRESHOLD})', linestyle='--')
plt.plot(results_df['rf_equity'], label=f'RF (Thresh {PROB_THRESHOLD})', linestyle=':')
plt.plot(results_df['xgb_equity'], label=f'XGB (Thresh {PROB_THRESHOLD})', linestyle='-.')
plt.title(f'Backtest V3: Strategy with {PROB_THRESHOLD*100:.0f}% Confidence Threshold')
plt.ylabel('Equity')
plt.legend()
plt.grid(True)
plt.savefig('v3_4_backtest_results.png')
plt.close()

print("\n--- Pipeline V3 Finished ---")