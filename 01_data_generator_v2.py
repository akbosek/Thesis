...
#                 RSI(30), Rolling Volatility(30), binary target.
...
#   Momentum            : btc_rsi_30        (Wilder RSI, normalised [0,1])
#   Regime / Vol        : btc_roll_vol_30  (21d rolling std of BTC returns)
...
#   Momentum            : btc_rsi_30        (unchanged — derived from log-ret)
#   Regime / Vol        : btc_roll_vol_30   (unchanged — derived from log-ret)
...
END_DATE:   str = "2026-01-02"  # exclusive — yfinance convention
...
TICKER_MAP: dict = {
    ...
    "DX-Y.NYB": "dxy",
    "^VIX":     "vix",
    "^TNX":     "us10y",   # US 10-Year Treasury Note Yield (macro rate signal)
}

VOLUME_ASSET: str = "btc"
ANCHOR_ASSET: str = "btc"

OUTPUT_CSV:     str = "processed_dataset_v3.csv"
OUTPUT_HEATMAP: str = "correlation_heatmap_v3.png"

RSI_PERIOD:        int = 30
VOL_WINDOW:        int = 30
VOL_RATIO_WIN:     int = 30
MACD_FAST:         int = 12   # MACD fast EMA period
MACD_SLOW:         int = 26   # MACD slow EMA period
MACD_SIGNAL:       int = 9    # MACD signal line EMA period
ATR_PERIOD:        int = 30   # ATR Wilder smoothing period
STOCH_RSI_PERIOD:  int = 30   # Stochastic RSI lookback period
BB_WINDOW:         int = 30   # Bollinger Band SMA/STD window
SMA50_WINDOW:      int = 50   # Close-to-SMA50 ratio window
MOM_WINDOW:        int = 30    # Momentum rate-of-change window (days)
RV_SHORT_WIN:      int = 5    # Realized vol skew — short window
RV_LONG_WIN:       int = 30   # Realized vol skew — long window
VWAP_WINDOW:       int = 30   # Rolling VWAP window for VWPD feature
AC1_WINDOW:        int = 30   # Rolling window for AR(1) autocorrelation
...
def fetch_close_prices(
    ...
    returning the Close price (and optionally Volume, High, Low).
    ...
    include_high_low  : If True, also returns '{short_name}_high' and
                        '{short_name}_low' (needed for ATR computation).
    ...
    if include_high_low:
        cols_to_keep += ["High", "Low"]
        rename_map["High"] = f"{short_name}_high"
        rename_map["Low"]  = f"{short_name}_low"
...
def merge_and_synchronize(frames: dict, anchor: str) -> pd.DataFrame:
    ...
    # INVARIANT: forward-fill ONLY. bfill would inject Monday's price into
    # Saturday/Sunday (look-ahead bias). Do not change this without re-auditing
    # the entire feature engineering pipeline.
...
def add_btc_rsi_30(df: pd.DataFrame) -> pd.DataFrame:
    """Add 'btc_rsi_30': Wilder's 30-day RSI on BTC log returns, in [0, 1]."""
    df["btc_rsi_30"] = _compute_wilder_rsi(df["btc_log_return"], period=RSI_PERIOD)
    log.info(
        "Feature added: btc_rsi_30  (Wilder RSI(%d), burn-in ≈ %d rows)",
        RSI_PERIOD, RSI_PERIOD + 1,
    )
    return df


def add_btc_rolling_vol_30(df: pd.DataFrame, window: int = VOL_WINDOW) -> pd.DataFrame:
    """Add 'btc_roll_vol_30': 30-day rolling std of BTC log returns."""
    df["btc_roll_vol_30"] = (
        df["btc_log_return"]
    ...
    log.info(
        "Feature added: btc_roll_vol_30  (window=%d, burn-in = %d rows)",
        window, window + 1,
    )
    return df


def add_btc_stoch_rsi_30(df: pd.DataFrame, period: int = STOCH_RSI_PERIOD) -> pd.DataFrame:
    """
    Add 'btc_stoch_rsi_30': Stochastic RSI = (RSI − RSI_min) / (RSI_max − RSI_min).

    Normalises the 30-day Wilder RSI to its own rolling range, producing a
    more sensitive oscillator bounded in [0, 1].  Values near 0 / 1 signal
    extreme oversold / overbought conditions with higher resolution than raw RSI.

    MUST run after add_btc_rsi_30() — depends on the 'btc_rsi_30' column.
    Adds ~period rows of additional burn-in on top of RSI's own burn-in.
    """
    rsi     = df["btc_rsi_30"]
    rsi_min = rsi.rolling(window=period, min_periods=period).min()
    rsi_max = rsi.rolling(window=period, min_periods=period).max()
    denom   = (rsi_max - rsi_min).replace(0.0, 1e-10)
    df["btc_stoch_rsi_30"] = ((rsi - rsi_min) / denom).clip(0.0, 1.0)
    log.info(
        "Feature added: btc_stoch_rsi_30  (Stoch-RSI(%d) on btc_rsi_14, "
        "total burn-in ≈ %d rows)",
        period, period + RSI_PERIOD + 1,
    )
    return df
...
def add_btc_macd(
    ...
    """
    Add 'btc_macd_hist': MACD histogram = (fast EMA − slow EMA) − signal EMA.

    The histogram sign flip marks momentum reversals, making it one of the
    most widely cited technical indicators in quantitative crypto literature.
    Not normalised by price (StandardScaler in the model script handles scale).

    Requires 'btc_close' — no additional burn-in beyond EWM warm-up.
    """
    close     = df["btc_close"]
    ...
    df["btc_macd_hist"] = macd_line - sig_line
    log.info(
        "Feature added: btc_macd_hist  (MACD hist EMA(%d)−EMA(%d)−EMA(%d))",
        fast, slow, signal,
    )
    return df


def add_btc_atr_30(df: pd.DataFrame, period: int = ATR_PERIOD) -> pd.DataFrame:
    """
    Add 'btc_atr_30_ratio': Wilder's ATR(30) divided by BTC closing price.

    True Range = max(High−Low, |High−PrevClose|, |Low−PrevClose|).
    ATR = Wilder EWM(True Range, alpha=1/period).
    Normalising by Close makes the ratio comparable across BTC's full price
    history ($200 in 2015 → $100 000+ in 2025).

    Requires 'btc_high', 'btc_low', 'btc_close' — fetch BTC with
    include_high_low=True.  Burns in over ~period rows; dropna() handles it.
    """
    high       = df["btc_high"]
    low        = df["btc_low"]
    close      = df["btc_close"]
    ...
    df["btc_atr_30_ratio"] = atr / close
    log.info(
        "Feature added: btc_atr_30_ratio  (Wilder ATR(%d) / Close)", period
    )
    return df


def add_btc_bollinger_pb(df: pd.DataFrame, window: int = BB_WINDOW) -> pd.DataFrame:
    """
    Add 'btc_bollinger_pb': Bollinger %B = (Close - Lower) / (Upper - Lower).

    Upper = SMA(window) + 2*STD(window), Lower = SMA(window) - 2*STD(window).
    Values > 1 or < 0 indicate price outside the bands (extreme moves); 0.5
    marks the midband (SMA). Burn-in: window rows. Depends on btc_close.
    """
    close = df["btc_close"]
    ...
    upper = sma + 2 * std
    lower = sma - 2 * std
    denom = (upper - lower).replace(0.0, 1e-10)
    df["btc_bollinger_pb"] = (close - lower) / denom
    log.info("Feature added: btc_bollinger_pb  (Bollinger %%B, window=%d)", window)
    return df


def add_btc_close_to_sma50(df: pd.DataFrame, window: int = SMA50_WINDOW) -> pd.DataFrame:
    """
    Add 'btc_close_to_sma50': Close / 50-day SMA.

    Values > 1 = price above trend; < 1 = below. Scale-invariant across
    BTC's full price history. Burn-in: 50 rows (binding constraint for the
    pipeline). Depends on btc_close.
    """
    close = df["btc_close"]
    sma50 = close.rolling(window=window, min_periods=window).mean()
    df["btc_close_to_sma50"] = close / sma50
    log.info("Feature added: btc_close_to_sma50  (Close / SMA(%d))", window)
    return df


def add_btc_momentum_30d(df: pd.DataFrame, window: int = MOM_WINDOW) -> pd.DataFrame:
    """
    Add 'btc_momentum_30d': 5-day rate of change = Close[t] / Close[t-30] - 1.

    Captures monthly momentum; positive = price higher than 30 days ago.
    Burn-in: 30 rows. Depends on btc_close.
    """
    close = df["btc_close"]
    df["btc_momentum_30d"] = close / close.shift(window) - 1
    log.info("Feature added: btc_momentum_30d  (%d-day ROC)", window)
    return df


def add_btc_rv_skew(
    df: pd.DataFrame,
    short: int = RV_SHORT_WIN,
    long:  int = RV_LONG_WIN,
) -> pd.DataFrame:
    """
    Add 'btc_rv_skew_5_30': short-term / long-term realized volatility ratio - 1.

    RV_short = RollingStd(btc_log_return, 5)
    RV_long  = RollingStd(btc_log_return, 30)
    Skew = RV_short / RV_long - 1

    Values > 0 signal vol expansion (spikes); < 0 signal vol compression
    (coiling). Orthogonal to the level of volatility captured by
    btc_roll_vol_30. Burn-in: long window rows. Depends on btc_log_return.
    """
    ret      = df["btc_log_return"]
    rv_short = ret.rolling(window=short, min_periods=short).std()
    rv_long  = ret.rolling(window=long,  min_periods=long).std()
    df["btc_rv_skew_5_30"] = rv_short / rv_long.replace(0.0, 1e-10) - 1
    log.info(
        "Feature added: btc_rv_skew_5_30  (RV(%d)/RV(%d) - 1)", short, long
    )
    return df


def add_btc_vwpd_30(df: pd.DataFrame, window: int = VWAP_WINDOW) -> pd.DataFrame:
    """
    Add 'btc_vwpd_30': Volume-Weighted Price Distance normalised by ATR.

    VWAP_30[t] = sum(Close * Volume, t-29:t) / sum(Volume, t-29:t)
    VWPD[t]    = (Close[t] - VWAP_30[t]) / ATR_14[t]

    Positive = price above rolling fair value; negative = below. ATR
    normalisation makes the signal scale-invariant across BTC history.
    MUST run after add_btc_atr_14 (depends on btc_atr_14_ratio).
    Burn-in: window rows. Depends on btc_close, btc_volume, btc_atr_14_ratio.
    """
    vol   = df["btc_volume"].replace(0.0, np.nan).ffill()
    close = df["btc_close"]
    pv    = close * vol
    ...
    atr   = df["btc_atr_30_ratio"] * close   # ratio → dollar ATR
    df["btc_vwpd_30"] = (close - vwap) / atr.replace(0.0, 1e-10)
    log.info(
        "Feature added: btc_vwpd_30  (VWAP(%d) distance / ATR(30))", window
    )
    return df


def add_btc_return_ac1(df: pd.DataFrame, window: int = AC1_WINDOW) -> pd.DataFrame:
    """
    Add 'btc_return_ac1_30': rolling 30-day AR(1) autocorrelation of log-returns.

    AC1[t] = Pearson corr(r[t-window+1:t], r[t-window:t-1])

    Positive = trending regime; negative = mean-reverting; near-zero = random
    walk. Helps the LSTM contextualise momentum features (RSI, MACD).
    Burn-in: window + 1 rows. Depends on btc_log_return.
    """
    ret    = df["btc_log_return"]
    lagged = ret.shift(1)
    df["btc_return_ac1_30"] = ret.rolling(window=window, min_periods=window).corr(lagged)
    log.info(
        "Feature added: btc_return_ac1_30  (%d-day rolling AR(1) correlation)", window
    )
    return df


def add_us10y_level(df: pd.DataFrame) -> pd.DataFrame:
    """
    Preserve the raw US 10-Year Treasury Note yield as 'us10y_level'.

    The yield LEVEL carries macro-regime information that the log-return
    discards: 5% indicates a tight-money environment regardless of whether
    it rose or fell yesterday.  Analogous treatment to vix_level.
    Values are in percent (e.g., 4.25 means 4.25%). Forward-filled on
    weekends and US holidays by merge_and_synchronize().

    MUST run after fetch_close_prices() for '^TNX' — depends on 'us10y_close'.
    """
    df["us10y_level"] = df["us10y_close"].copy()
    log.info("Feature added: us10y_level  (US 10Y Treasury yield, raw level, %%)")
    return df
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    5. add_btc_rsi_30         – Consumes btc_log_return; MUST follow step 2.
    6. add_btc_rolling_vol_30 – Consumes btc_log_return; MUST follow step 2.
    ...
    Output columns — USE_RAW_PRICES=False  (11 features + 1 target):
        ...
        btc_volume_ratio | btc_rsi_30 | btc_roll_vol_30 | target

    Output columns — USE_RAW_PRICES=True  (10 features + 1 aux ref + 1 target):
        is_weekend | btc_close | spy_close | gold_close | nvda_close |
        dxy_close | vix_level | btc_volume_ratio | btc_rsi_30 |
        btc_roll_vol_30 | btc_log_return [ref] | target
    """
    df = df.copy()

    # ── Steps 1–11: compute all intermediate columns ──────────────────────────
    df = add_weekend_flag(df)
    df = add_log_returns(df)           # MUST precede vix_level / us10y_level
    df = add_vix_level(df)             # copies vix_close → vix_level
    df = add_us10y_level(df)           # copies us10y_close → us10y_level
    df = add_btc_volume_ratio(df)      # consumes btc_volume
    df = add_btc_rsi_30(df)            # consumes btc_log_return
    df = add_btc_rolling_vol_30(df)    # consumes btc_log_return
    df = add_btc_stoch_rsi_30(df)      # MUST follow add_btc_rsi_14
    df = add_btc_macd(df)              # consumes btc_close
    df = add_btc_atr_30(df)            # consumes btc_high, btc_low, btc_close
    df = add_btc_bollinger_pb(df)      # consumes btc_close
    df = add_btc_close_to_sma50(df)    # consumes btc_close; burn-in = 50 rows (binding)
    df = add_btc_momentum_30d(df)       # consumes btc_close
    df = add_btc_rv_skew(df)           # consumes btc_log_return
    df = add_btc_vwpd_30(df)          # consumes btc_close, btc_volume, btc_atr_14_ratio
    df = add_btc_return_ac1(df)        # consumes btc_log_return
    ...
    if USE_RAW_PRICES:
        ...
        # Drop us10y_close (replaced by us10y_level) and btc H/L (ATR intermediate).
        ohlc_and_ref_cols = ["btc_high", "btc_low", "us10y_close"]
        df.drop(
            columns=log_return_cols_to_drop + ohlc_and_ref_cols,
        ...
        # Also drop btc H/L (ATR intermediate) and us10y_log_return (we use
        # us10y_level instead of the log-return in both modes).
        close_cols  = [f"{name}_close" for name in TICKER_MAP.values()]
        volume_cols = [f"{VOLUME_ASSET}_volume"]
        ohlc_cols   = ["btc_high", "btc_low"]
        extra_cols  = ["us10y_log_return"]
        df.drop(
            columns=close_cols + volume_cols + ohlc_cols + extra_cols,
            inplace=True, errors="ignore",
        )
...
_HEATMAP_LABEL_MAP: dict = {
    ...
    "btc_stoch_rsi_14": "BTC\nStochRSI",
    "btc_macd_hist":    "BTC MACD\nHistogram",
    "btc_atr_14_ratio":    "BTC ATR\nRatio",
    "btc_bollinger_pb":    "BTC Bollinger",
    "btc_close_to_sma50":  "BTC/SMA50\nRatio",
    "btc_momentum_5d":     "BTC Mom.\n(5d)",
    "btc_rv_skew_5_21":    "BTC RV\nSkew(5/30)",
    "btc_vwpd_20":         "BTC VWPD",
    "btc_return_ac1_20":   "BTC AC1",
...
def main() -> None:
    ...
    for col in [
        "vix_level", "us10y_level", "btc_volume_ratio",
        "btc_rsi_30", "btc_roll_vol_30", "btc_stoch_rsi_30",
        "btc_macd_hist", "btc_atr_30_ratio",
    ]:
    ...
    log.info("01_data_generator_v3.py  —  PIPELINE COMPLETE")
